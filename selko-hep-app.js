...
async function loadCustomExercises() {
  ...
  if (error) { console.error(error); state.exerciseLibrary = [...EXERCISE_LIBRARY]; return; }

  const custom = (data || []).map(row => ({
    id: row.id, name: row.name, image: row.image || '',
    keywords: row.keywords || [], instructions: row.instructions || '',
    date_added: row.created_at, added_by: 'custom', is_custom: true,
  }));
...
function renderAdminExerciseList() {
  ...
  if (custom.length === 0) { list.innerHTML = ''; empty.classList.remove('hidden'); return; }
  ...
  list.querySelectorAll('.btn-delete-custom-ex').forEach(btn => {
    btn.addEventListener('click', async (e) => {
      e.stopPropagation();
      const ex = custom.find(c => c.id === btn.dataset.id);
      if (!confirm(`Delete "${ex?.name}"? This can't be undone.`)) return;
      const { error } = await db.from('hep_custom_exercises').delete().eq('id', btn.dataset.id);
      if (error) { console.error(error); showToast('Could not delete — try again'); return; }
      await loadCustomExercises(); renderAdminExerciseList(); showToast('Exercise deleted');
    });
  });
}
...
async function loadAndRenderUserList() {
  ...
  if (error) { console.error(error); list.innerHTML = '<div class="screen-hint" style="padding:12px 0;color:var(--danger);">Could not load users.</div>'; return; }
  const users = data || [];
  if (users.length === 0) { list.innerHTML = ''; empty.classList.remove('hidden'); return; }
  empty.classList.add('hidden');
  list.innerHTML = users.map(u => {
    const badge = u.is_super_admin
      ? '<span style="font-size:10px;background:var(--selko-navy);color:#fff;border-radius:4px;padding:2px 6px;font-family:var(--font-head);font-weight:600;margin-left:6px;">ADMIN</span>'
      : u.role === 'admin'
        ? '<span style="font-size:10px;background:var(--selko-teal);color:#fff;border-radius:4px;padding:2px 6px;font-family:var(--font-head);font-weight:600;margin-left:6px;">ADMIN</span>'
        : '';
    return `<div class="program-item">
      <div class="program-item-info">
        <div class="program-item-name" style="display:flex;align-items:center;gap:4px;">${u.full_name||'—'}${badge}</div>
...
function openInviteModal() {
  ...
  msgEl.classList.add('hidden'); msgEl.className = 'alert hidden';
...
async function handleSendInvite() {
  ...
  if (!name) { msgEl.textContent = 'Please enter the clinician\'s full name.'; msgEl.className = 'alert alert-error'; msgEl.classList.remove('hidden'); return; }
  if (!email || !email.includes('@')) { msgEl.textContent = 'Please enter a valid email address.'; msgEl.className = 'alert alert-error'; msgEl.classList.remove('hidden'); return; }
  btn.disabled = true; btn.textContent = 'Sending…';
  try {
    const { data, error } = await db.functions.invoke('invite-staff', {
      body: { name, email, company_id: state.companyId, role: 'clinician', app: 'hep' },
    });
    if (error || data?.error) throw new Error(error?.message || data?.error || 'Unknown error');
    msgEl.textContent = `Invite sent to ${email}! They'll receive an email to set up their account.`;
    msgEl.className = 'alert alert-success'; msgEl.classList.remove('hidden');
    nameEl.value = ''; emailEl.value = '';
    loadAndRenderUserList();
    setTimeout(() => { document.getElementById('invite-user-modal').classList.add('hidden'); }, 3000);
  } catch (err) {
    console.error('Invite error:', err);
    msgEl.textContent = 'Could not send invite — ' + (err.message || 'please try again.');
    msgEl.className = 'alert alert-error'; msgEl.classList.remove('hidden');
...
async function handleSaveNewExercise() {
  ...
  if (!name) { msgEl.textContent = 'Please enter an exercise name.'; msgEl.className = 'alert alert-error'; msgEl.classList.remove('hidden'); return; }
  ...
  const { error } = await db.from('hep_custom_exercises').insert([{
    company_id: state.companyId, created_by: state.user.id,
    name, image: imageUrl, instructions: instrEl.value.trim(), keywords,
  }]);
  btn.disabled = false; btn.textContent = 'Save exercise';
  if (error) { console.error(error); msgEl.textContent = 'Could not save exercise — try again.'; msgEl.className = 'alert alert-error'; msgEl.classList.remove('hidden'); return; }
...
/* ═══════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════ */
const T = {
  en: { welcome:'Welcome!', programReady:'Your personalized exercise program is ready.', viewProgram:'View my program', footer:'Reach out to your provider with any questions or concerns.', myExercises:'My exercises', markDone:'✓ Mark as done', completed:'✓ Completed today', back:'← Back' },
  es: { welcome:'¡Bienvenido!', programReady:'Su programa de ejercicios personalizado está listo.', viewProgram:'Ver mi programa', footer:'Comuníquese con su proveedor si tiene preguntas o inquietudes.', myExercises:'Mis ejercicios', markDone:'✓ Marcar como hecho', completed:'✓ Completado hoy', back:'← Atrás' },
};
...
function showScreen(id) {
  document.querySelectorAll('.screen').forEach(s => { s.classList.remove('active'); s.classList.add('hidden'); s.style.display = 'none'; });
  const el = document.getElementById(id);
  if (el) { el.classList.remove('hidden'); el.style.display = 'flex'; el.classList.add('active'); }
  const appScreens = ['screen-home','screen-exercises','screen-current','screen-programs','screen-program-detail','screen-admin'];
  const header = document.getElementById('app-header');
  if (appScreens.includes(id)) { header.classList.remove('hidden'); header.style.display = 'flex'; }
  else { header.classList.add('hidden'); header.style.display = 'none'; }
}
...
function showToast(msg, duration = 2800) {
  ...
  toast._timer = setTimeout(() => { toast.classList.add('fade-out'); setTimeout(() => toast.classList.add('hidden'), 320); }, duration);
}
...
function updateBadges() {
  ...
  if (hint) hint.textContent = `${n} exercise${n !== 1 ? 's' : ''} selected this session. Adjust parameters below.`;
}
...
async function handleLogin() {
  ...
  state.user = data.user; onLoginSuccess();
}
...
async function handleSetNewPassword() {
  ...
  state.user = session.user; await onLoginSuccess();
}

async function handleLogout() {
  await db.auth.signOut();
  state.user = null; state.currentSession = []; state.programs = [];
  closeNav(); showScreen('screen-login');
}
...
async function onLoginSuccess() {
  ...
  loadPrograms(); applyAdminVisibility(); showScreen('screen-home');
}
...
function openNav() {
  ...
  nav.classList.remove('hidden'); nav.removeAttribute('inert'); nav.removeAttribute('aria-hidden');
...
function closeNav() {
  ...
  setTimeout(() => { nav.classList.add('hidden'); nav.setAttribute('inert',''); nav.setAttribute('aria-hidden','true'); }, 260);
}
...
function onExerciseCardClick(exId) {
  ...
  if (inSession) { addBtn.classList.add('hidden'); removeBtn.classList.remove('hidden'); }
  else           { addBtn.classList.remove('hidden'); removeBtn.classList.add('hidden'); }
    ...
    return `
      <div class="ex-list-item" data-id="${s.exerciseId}" data-idx="${idx}">
        ...
        <div style="padding:0 14px 12px;display:flex;gap:8px;flex-wrap:wrap;">
          ...
          ${s.patientPhoto?`<span style="font-size:12px;color:var(--selko-navy);">✓ Photo added</span>`:''}
          ${s.patientVideo?`<span style="font-size:12px;color:var(--selko-navy);">✓ Video added</span>`:''}
        </div>
      </div>`;
  }).join('');
...
/* ─── Save program without sending ──────────────────────────── */
function openSaveProgramModal() {
  if (state.currentSession.length === 0) {
    showToast('Add at least one exercise before saving');
    return;
  }
  const msgEl = document.getElementById('save-program-msg');
  msgEl.classList.add('hidden');
  document.getElementById('save-program-name').value = '';
  openModal('save-program-modal');
}

async function handleSaveProgram() {
  const nameInput = document.getElementById('save-program-name');
  const msgEl     = document.getElementById('save-program-msg');
  const btn       = document.getElementById('btn-confirm-save-program');
  msgEl.classList.add('hidden');
  ...
  const payload = {
    ...
    code:            makeCode(),   // code generated but not sent yet
    phone:           '',           // no phone — will be entered when sending later
    expiry,
    compliance_days: 0,
    created_at:      new Date().toISOString(),
  };

  const { error } = await db.from('hep_programs').insert([payload]);
  btn.disabled = false; btn.textContent = 'Save';

  if (error) {
    console.error(error);
    msgEl.textContent = 'Could not save program — try again.';
    msgEl.className = 'alert alert-error';
    msgEl.classList.remove('hidden');
    return;
  }

  // Reset session, close modal, stay on exercises screen
  closeModal('save-program-modal');
  state.currentSession = [];
  updateBadges();
  renderExerciseGrid(document.getElementById('exercise-search').value);
  await loadPrograms();
  showToast(`"${name}" saved to My Programs`);
}
...
function renderPatientExerciseGrid() {
  ...
  grid.innerHTML = (prog.exercises||[]).map(s => {
    const ex = getExercise(s.exerciseId); if (!ex) return '';
...
function openPatientExercise(exId) {
  ...
  if (s.patientVideo) { videoEl.src=s.patientVideo; videoWrap.classList.remove('hidden'); }
  else { videoWrap.classList.add('hidden'); videoEl.src=''; }
  const params = [s.sets,s.reps,s.freq].filter(Boolean).join(' · ');
...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('invite-email').addEventListener('keydown', e => { if (e.key==='Enter') handleSendInvite(); });

  /* ─── SAVE PROGRAM MODAL ──────────────────────────────────── */
  document.getElementById('btn-save-program').addEventListener('click', openSaveProgramModal);
  document.getElementById('btn-confirm-save-program').addEventListener('click', handleSaveProgram);
  document.getElementById('btn-cancel-save-program').addEventListener('click', () => closeModal('save-program-modal'));
  document.getElementById('save-program-name').addEventListener('keydown', e => { if (e.key==='Enter') handleSaveProgram(); });
